...
public class Home extends BaseActivity implements HomeView {
    ...
    final ArrayList<LatLng> driversLocation = new ArrayList<>();
    ...
    public MapFragmentCallback callback = new MapFragmentCallback() {
        ...
        public void onExpectedPlaceSelected(Place expectedPlace) {
            ...
            if (expectedPlace != null) {
                myPlace = expectedPlace;
                onSelectPlace(expectedPlace);
                addDynamicShortcut(expectedPlace);
                ActionParams actionParams = new ActionParamsBuilder().setCollectionId(collectionId != null ? collectionId : UUID.randomUUID().toString()).setType(Action.ACTION_TYPE_VISIT).setExpectedPlace(expectedPlace).build();
                HyperTrack.createAndAssignAction(actionParams, new HyperTrackCallback() {
                    ...
                    public void onSuccess(@NonNull SuccessResponse successResponse) {
                        if (successResponse.getResponseObject() != null) {
                            ...
                            Toast.makeText(getApplicationContext(), "Live Location successfully shared back", Toast.LENGTH_SHORT).show();
                        }
                    }
...
//        String locations = googleMap.getMyLocation().getLatitude() + "," + googleMap.getMyLocation().getLongitude();
//        Log.e("LOCATION",""+locations);
        LatLng latLng;
...
//
                String locations = googleMap.getMyLocation().getLatitude() + "," + googleMap.getMyLocation().getLongitude();
                Log.e("LOCATION", "" + locations);
                showNearByDrivers(this, locations);

                for (int i = 0; i < driversLocation.size(); i++) {
                    drawMarker(driversLocation.get(i));
    ...
    private void updateMapView() {
        ...
        if (currentLocationMarker != null) {
            ...
            //Call to show near by drivers on the consumer location.
//            String locations = mMap.getMyLocation().getLatitude() + "," + mMap.getMyLocation().getLongitude();
//            Log.e("LOCATION",""+locations);
//            showNearByDrivers(this,locations);
//
//            for(int i=0;i<driversLocation.size();i++)
...
//            }
    ...
    private void startAction() {
        ...
        ActionParams actionParams = new ActionParamsBuilder().setExpectedPlace(expectedPlace).setType(Action.ACTION_TYPE_PICKUP).setLookupId(lookupId).build();
        HyperTrack.createAndAssignAction(actionParams, new HyperTrackCallback() {
            @Override
            public void onSuccess(@NonNull SuccessResponse successResponse) {
                Action action = (Action) successResponse.getResponseObject();
                ...
                public void onSuccess(@NonNull SuccessResponse response) {
                    if (htMapFragment != null) {
                        ...
                        ActionParams actionParams = new ActionParamsBuilder()
                                ...
                                .setType(Action.ACTION_TYPE_DROPOFF).setExpectedPlace(expectedPlace).build();
                        HyperTrack.createAndAssignAction(actionParams, new HyperTrackCallback() {
                            @Override
                            public void onSuccess(@NonNull SuccessResponse successResponse) {
                                if (successResponse.getResponseObject() != null) {
...
//        Place expectedPlace = new Place().setLocation(mMap.getMyLocation().getLatitude(), mMap.getMyLocation().getLongitude())
//                .setAddress(String.valueOf(myPlace));
//
//        Toast.makeText(getApplicationContext(), "" + myPlace + "expectedPlace" + expectedPlace, Toast.LENGTH_LONG).show();
//    }

    //    protected ArrayList<HyperTrackLocation> showNearByDrivers(Context context) throws IOException {
//        final ArrayList<HyperTrackLocation> driverLocation=new ArrayList<>();
//        final HyperTrackService getVerifyCodeService = HyperTrackServiceGenerator.createService(HyperTrackService.class
//                , context);
//        String location=mMap.getMyLocation().getLatitude()+","+mMap.getMyLocation().getLongitude();
//        Call<List<User>> call = getVerifyCodeService.showNearByDrivers(location);
...
//                    {
//                    driverLocation.add( users.get(i).getLastLocation());
//
//                    }
//
//                }
...
//    }
    ...
    protected void showNearByDrivers(Context context, String location) {
        ...
        call.enqueue(new Callback<List<User>>() {
            ...
            public void onResponse(Call<List<User>> call, Response<List<User>> response) {
                List<User> users = (List<User>) response.body();
                for (int i = 0; i < users.size(); i++) {
                    driversLocation.add(users.get(i).getLastLocation().getLatLng());
                    Toast.makeText(getApplicationContext(), "drivers location" + driversLocation, Toast.LENGTH_LONG).show();
                    Log.e("DRIVERS LOCATION", "" + driversLocation);
    ...
    private void drawMarker(LatLng point) {
        ...
        Toast.makeText(getApplicationContext(), "position values are" + markerOptions.position(point), Toast.LENGTH_LONG).show();
        Log.e("POSITION VALUES ON MAP", "" + markerOptions.position(point));
    }
}
